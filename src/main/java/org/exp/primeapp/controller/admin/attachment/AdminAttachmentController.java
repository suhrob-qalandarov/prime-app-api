...
public class AdminAttachmentController {
    ...
    @Operation(summary = "Upload single file", description = "Upload a single file attachment", security = @SecurityRequirement(name = "Authorization"))
    @PostMapping(value = "/oneupload", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @PreAuthorize("hasAnyAuthority('ADMIN', 'SUPER_ADMIN')")
    public ResponseEntity<AttachmentRes> uploadFile(
            @Parameter(description = "File to upload", required = true) @RequestParam("file") @Schema(type = "string", format = "binary") MultipartFile file) {
    ...
    @Operation(summary = "Upload multiple files", description = "Upload multiple file attachments", security = @SecurityRequirement(name = "Authorization"))
    @PostMapping(value = "/multiupload", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @PreAuthorize("hasAnyAuthority('ADMIN', 'SUPER_ADMIN')")
    public ResponseEntity<List<String>> uploadFiles(
            @Parameter(description = "Files to upload", required = true) @RequestParam("files") @Schema(type = "array", format = "binary") MultipartFile[] files) {
    ...
    @Operation(summary = "Update attachment file", description = "Update an existing attachment file", security = @SecurityRequirement(name = "Authorization"))
